...
public class CardDetailsActivity extends Activity {
    ...
    private static final String PACKAGE_NAME = CardDetailsActivity.class.getPackage().getName();
    ...
    float mWidthScale;
    float mHeightScale;
    private TextView mTextTitle, mTextData;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mTopLevelLayout = (FrameLayout) findViewById(R.id.cardTopLevelLayout);
        mShadowLayout = (ShadowLayout) findViewById(R.id.cardShadowLayout);
        mTextTitle = (TextView) findViewById(R.id.titleTag);
        mTextData = (TextView) findViewById(R.id.descriptionTag);
        card = (CardView)findViewById(R.id.cardDetailsID);
        mImageView = (ImageView)findViewById(R.id.imageViewcARD);

        // Retrieve the data we need for the picture/description to display and
        // the thumbnail to animate it from
        Bundle bundle = getIntent().getExtras();

//        Bitmap bitmap = BitmapUtils.getBitmap(getResources(), bundle.getInt(PACKAGE_NAME + ".resourceId"));
        Bitmap bitmap = BitmapUtils.getBitmap(getResources(), R.drawable.tag);

        String titleCard = bundle.getString(PACKAGE_NAME + ".titleCard");
        String descriptionCard = bundle.getString(PACKAGE_NAME + ".descriptionCard");
        ...
        Log.d(null, "title card "+titleCard);
        Log.d(null, "title data "+descriptionCard);

        mBitmapDrawable = new BitmapDrawable(getResources(), bitmap);
        mImageView.setImageDrawable(mBitmapDrawable);
        mTextTitle.setText(titleCard);
        mTextData.setText(descriptionCard);
        ...
        if (savedInstanceState == null) {
            ...
            observer.addOnPreDrawListener(new ViewTreeObserver.OnPreDrawListener() {
                ...
                public boolean onPreDraw() {
                    ...
                    card.getLocationOnScreen(screenLocation);
                    mLeftDelta = thumbnailLeft - screenLocation[0];
                    mTopDelta = thumbnailTop - screenLocation[1];

                    // Scale factors to make the large version the same size as the thumbnail
                    mWidthScale = (float) thumbnailWidth / card.getWidth();
                    mHeightScale = (float) thumbnailHeight / card.getHeight();
    ...
    public void runEnterAnimation() {
        ...
        card.setPivotX(0);
        card.setPivotY(0);
        card.setScaleX(mWidthScale);
        card.setScaleY(mHeightScale);
        card.setTranslationX(mLeftDelta);
        card.setTranslationY(mTopDelta);

        // We'll fade the text in later
        card.setAlpha(0);

        // Animate scale and translation to go from thumbnail to full size
        card.animate().setDuration(duration).
                scaleX(1).scaleY(1).
                ...
                withEndAction(new Runnable() {
                    public void run() {
                        ...
                        card.animate().setDuration(duration / 2).
        ...
        ObjectAnimator colorizer = ObjectAnimator.ofFloat(CardDetailsActivity.this,
                "saturation", 0, 1);
    ...
    public void runExitAnimation(final Runnable endAction) {
        ...
        card.animate().translationY(-card.getHeight()).alpha(0).
                ...
                withEndAction(new Runnable() {
                    public void run() {
                        ...
                        ObjectAnimator colorizer =
                                ObjectAnimator.ofFloat(CardDetailsActivity.this,
                                        "saturation", 1, 0);
                        colorizer.setDuration(duration);
                        colorizer.start();
                    }
